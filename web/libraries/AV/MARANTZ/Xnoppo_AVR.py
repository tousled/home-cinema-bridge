# FILE FOR MARANTZ
from lib.devices.av.marantz import MarantzAvReceiver
...
def get_hdmi_list():
    ...
    hdmi_intput_list=add_hdmi(5,"SAT",'SISAT\n',hdmi_intput_list)
    hdmi_intput_list=add_hdmi(5,"PEPE",'SICBL\n',hdmi_intput_list)
    hdmi_intput_list=add_hdmi(6,"GAME",'SIGAME\n',hdmi_intput_list)
...
def av_check_power(config):
    return MarantzAvReceiver(config).power_on()


def av_change_hdmi(config):
    return MarantzAvReceiver(config).change_hdmi()


def av_power_off(config):
    return MarantzAvReceiver(config).power_off()
